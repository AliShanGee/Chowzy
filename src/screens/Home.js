...
export default function Home() {
  ...
  // Pre-group items by category and deduplicate by name in O(N)
  const groupedItems = useMemo(() => {
    ...
    // Convert internal maps to arrays for easier mapping in render
    const result = new Map();
    for (const [catName, itemsMap] of map) {
      result.set(catName, Array.from(itemsMap.values()));
    }
    return result;
  }, [foodItem]);
