...
export default function Home() {
  ...
  // Memoize unique categories to avoid O(N^2) deduplication on every render
  ...
  // Pre-process, filter and group food items by category to avoid expensive nested filtering in render
  // This reduces complexity from O(Categories * Items^2) to O(Items)
  const groupedItems = useMemo(() => {
    ...
    foodItem.forEach(item => {
      ...
      // Use item name as key for deduplication as per original logic
    ...
    grouped.forEach((itemsMap, category) => {
      result.set(category, Array.from(itemsMap.values()));
    });
    return result;
  }, [foodItem, search]);
            ...
            return (
              <>
                {currentCategories.map((data) => {
                  const itemsInCategory = groupedItems.get(data.CategoryName) || [];
                  return (
                    <div className='row mb-3' key={data._id}>
                      ...
                      {foodItem.length > 0
                        ? (itemsInCategory.length > 0
                            ...
                            : null // Hide category if no items match search, or keep original behavior
                          )
