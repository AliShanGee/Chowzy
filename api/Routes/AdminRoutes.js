...
const storage = (isNode && multer && path && fs) ? multer.diskStorage({
...
router.post('/reels', (isNode && upload) ? upload.single('video') : (req, res, next) => next(), async (req, res) => {
    try {
...
router.put('/reels/:id', (isNode && upload) ? upload.single('video') : (req, res, next) => next(), async (req, res) => {
    try {
...
router.delete('/reels/:id', async (req, res) => {
    try {
        ...
        if (isNode && path && fs && reel && reel.videoUrl && reel.videoUrl.startsWith('/uploads/')) {
