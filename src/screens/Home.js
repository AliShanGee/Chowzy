...
export default function Home() {
  ...
  // Pre-calculate unique categories to avoid redundant filtering on every render
  ...
  const totalPages = useMemo(() => Math.ceil(uniqueCategories.length / itemsPerPage), [uniqueCategories]);

  // Group and deduplicate food items by CategoryName in O(N) time
  // This avoids expensive O(N^2) filtering and deduplication within the render loop
  const groupedItems = useMemo(() => {
    ...
    foodItem.forEach(item => {
      ...
      const namesSet = seenNamesPerCategory.get(item.CategoryName);
              ...
              <>
                {currentCategories.map((data) => {
                  ...
                  const filteredItems = search === ""
                    ? categoryItems
                    : categoryItems.filter(item => item.name.toLowerCase().includes(searchLower));
