...
export default function Home() {
  ...
  // Memoize unique categories to prevent re-filtering on every render
  ...
  // Group and deduplicate food items by CategoryName for O(1) category lookup and O(Items) overall complexity
  const groupedFoodItems = useMemo(() => {
    ...
    foodItem.forEach(item => {
      ...
      // Deduplicate by name as per existing logic
            ...
            return (
              <>
                {currentCategories.map((data) => {
                  const catItemsMap = groupedFoodItems.get(data.CategoryName);
                  const filteredItems = [];

                  if (catItemsMap) {
                    for (const item of catItemsMap.values()) {
