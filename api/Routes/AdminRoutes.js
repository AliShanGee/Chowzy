...
}) : { single: () => (req, res, next) => next() }; // Dummy for non-Node
...
router.post('/reels', isNode ? upload.single('video') : (req, res, next) => next(), async (req, res) => {
    try {
...
router.put('/reels/:id', isNode ? upload.single('video') : (req, res, next) => next(), async (req, res) => {
    try {
