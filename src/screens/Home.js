...
export default function Home() {
  ...
  // Bolt Optimization: Memoize unique categories to avoid O(N^2) filter/findIndex in render
  ...
  // Bolt Optimization: Pre-group and deduplicate items by CategoryName in O(N)
  // This avoids nested O(N*M) filtering during the render loop
  const groupedItems = useMemo(() => {
    ...
    foodItem.forEach((item) => {
      ...
      // Deduplicate by name as per original logic
      if (!catMap.has(item.name)) {
        catMap.set(item.name, item);
      }
    });

    // Convert to Map of Arrays for efficient access
    const result = new Map();
    map.forEach((itemMap, catName) => {
      result.set(catName, Array.from(itemMap.values()));
    });
    return result;
  }, [foodItem]);
