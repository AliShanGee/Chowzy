...
export default function Home() {
  ...
  // Performance Optimization: Memoize unique categories calculation - O(N)
  ...
  // Performance Optimization: Group and deduplicate items by category in one pass - O(Items)
  const groupedFoodItems = useMemo(() => {
    const grouped = {};
    foodItem.forEach(item => {
      if (!item.name) return;
      if (!grouped[item.CategoryName]) {
        grouped[item.CategoryName] = new Map();
      }
      // Deduplicate by name: only keep the first occurrence
      if (!grouped[item.CategoryName].has(item.name)) {
        grouped[item.CategoryName].set(item.name, item);
      }
    });
    ...
    Object.keys(grouped).forEach(cat => {
      result[cat] = Array.from(grouped[cat].values());
    });
    return result;
  }, [foodItem]);
            ...
            return (
              <>
                {currentCategories.map((data) => {
                  const itemsInCategory = groupedFoodItems[data.CategoryName] || [];
