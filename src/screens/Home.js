...
export default function Home() {
  ...
  // Bolt Optimization: Pre-group and deduplicate food items to avoid O(N^2) complexity in render loop
  const processedData = useMemo(() => {
    if (foodCat.length === 0) return { uniqueCategories: [], groupedItems: {} };
    ...
    foodItem.forEach(item => {
      ...
      if (!grouped[item.CategoryName].some(existing => existing.name === item.name)) {
          ...
          {uniqueCategories.length > 0 && (
            <>
              {currentCategories.map((data) => {
                const filteredItems = (groupedItems[data.CategoryName] || [])
